...
def compute_subject_stats(df: pd.DataFrame, subject_col: str) -> pd.DataFrame:
    ...
    if subject_col not in df.columns:
        ...
        return pd.DataFrame()
    g = df.groupby(subject_col)["label_int"]
    baseline_wins = g.apply(lambda s: (s == 1).sum())
    alt_wins = g.apply(lambda s: (s == 0).sum())
    ties = g.apply(lambda s: (s == -1).sum())
...
def generate_plots(df: pd.DataFrame, subject_stats: pd.DataFrame, metrics: list[str], top_k: int, bins: int, out_dir: Path, save: bool):
    ...
    if not subject_stats.empty:
        # Top K
        top = subject_stats.head(top_k).reset_index()
        plt.figure()
        subj_col = subject_stats.index.name or "subject"
        plt.bar(top[subj_col], top["baseline_win_rate"])
        plt.xticks(rotation=60, ha="right")
        plt.ylabel("Baseline win rate")
        plt.title(f"Top {top_k} subjects where canonical wins")
        plt.tight_layout()
        if save:
            plt.savefig(figs_dir / "bar_top_subjects.png", dpi=150)
            plt.close()
        else:
            plt.show()
        # Bottom K
        bottom = subject_stats.tail(top_k).reset_index()
        plt.figure()
        plt.bar(bottom[subj_col], bottom["baseline_win_rate"])
        plt.xticks(rotation=60, ha="right")
        plt.ylabel("Baseline win rate")
        plt.title(f"Bottom {top_k} subjects (alternative wins)")
        plt.tight_layout()
        if save:
            plt.savefig(figs_dir / "bar_bottom_subjects.png", dpi=150)
            plt.close()
        else:
            plt.show()
...
def main():
    ...
    if not args.no_plots:
        ...
        generate_plots(df_analysis, subject_stats, args.metrics, args.top_k, args.bins, out_dir, save=args.save_plots)
