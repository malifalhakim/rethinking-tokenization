...
class BPEUndertrainedNormTokenizer(BPENormTokenizer):
    """
    A wrapper for BPE-based tokenizers that generates a single tokenization 
    that aims to maximize the average token norm, specifically focusing on words
    ...
    def __init__(self, tokenizer, token_norm: TokenNorm, threshold: str = "strong_verified"):
        ...
        """
        super().__init__(tokenizer, token_norm)
    ...
    def generate_best_tokenization(self, text: str) -> List[str]:
        ...
        maximize the overall average token norm by optimizing word by word. Only
