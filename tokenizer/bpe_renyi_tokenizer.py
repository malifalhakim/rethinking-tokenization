...
class BPERenyiTokenizer(BPEAlternativeTokenizerFiltered):
    """
    A wrapper for BPE-based tokenizers that generates a single tokenization 
    that aims to maximize the average token renyi, while also filtering out
    "random BPE" segmentations.
    """

    def __init__(self, tokenizer):
        """
        Initializes the renyi-based tokenizer.
        """
    ...
    def _find_best_word_tokenization(self, word: str, canonical_tokenization: List[str]) -> List[str]:
        """
        Finds the tokenization for a single word that maximizes token renyi and is
        ...
        max_score = renyi_score(canonical_tokenization)
        ...
        for tokenization in all_tokenizations:
            ...
            score = renyi_score(tokenization)
            if score > max_score:
                max_score = score
    ...
    def generate_best_tokenization(self, text: str) -> List[str]:
        ...
        maximize the overall average token renyi by optimizing word by word.
        """
    ...
    def generate_alternatives(self, text: str, n: int = 1) -> List[List[str]]:
        ...
        it returns a list containing only the single best tokenization based on renyi.
